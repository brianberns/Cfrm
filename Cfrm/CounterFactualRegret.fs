...
module CounterFactualRegret =
    ...
    /// Main CFR loop.
    let rec private loop cInfoSetMap reachProbs (gameState : GameState<_>) =
                ...
                if reachProbs |> Vector.forall ((=) 0.0) then
                    ...
                    match legalActions.Length with
                        ...
                        | 1 ->   // trivial case
                            ...
                            loop cInfoSetMap reachProbs nextState
                        | _ -> cfrCore cInfoSetMap reachProbs gameState legalActions
    ...
    /// Core CFR algorithm.
    and private cfrCore cInfoSetMap reachProbs gameState legalActions =
        ...
        cInfoSetMap |> ConcurrentInfoSetMap.accumulate key regrets strategy
    ...
    let minimizeBatch numIterations batch =
        ...
        let cInfoSetMap = ConcurrentInfoSetMap(batch.InfoSetMap)
        ...
        Parallel.For(
            ...
            (fun iter _ localUtils ->
                let utils =
                    batch.GetInitialState(iter)
                        |> loop cInfoSetMap (DenseVector.create numPlayers 1.0)
                localUtils + utils),
            fun localUtils ->
        ...
        let infoSetMap = ConcurrentInfoSetMap.toMap cInfoSetMap
        { batch with
            Utilities = utilities
            InfoSetMap = infoSetMap
